export function animateE(hand, scene) {
    const fingers = {
        thumb: [],
        index: [],
        middle: [],
    ...
    // Apply the 'E' gesture 
    fingers.index.forEach(bone => animateBone(bone, 1.0));   // Extend index finger
    fingers.middle.forEach(bone => animateBone(bone, 1.0));  // Extend middle finger
    fingers.ring.forEach(bone => animateBone(bone, 1.0));    // Extend ring finger
    fingers.pinky.forEach(bone => animateBone(bone, 1.0));   // Extend pinky finger
    fingers.thumb.forEach(bone => animateBone(bone, 0));    // Adduct thumb
 }
