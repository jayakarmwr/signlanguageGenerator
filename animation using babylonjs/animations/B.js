// B.js
export function animateB(hand, scene) {
    const fingers = {
        thumb: [],
        index: [],
        middle: [],
    ...
    // Apply the 'B' gesture
    fingers.index.forEach(bone => animateBone(bone, 0, 0, 0));   // Extend index finger
    fingers.middle.forEach(bone => animateBone(bone,0, 0, 0));  // Extend middle finger
    fingers.ring.forEach(bone => animateBone(bone, 0, 0, 0));    // Extend ring finger
    fingers.pinky.forEach(bone => animateBone(bone, 0, 0, 0));   // Extend pinky finger

    // Thumb adjustments
    fingers.thumb.forEach(bone => animateBone(bone, 0.3, -0.8, 0));    // Adduct thumb and rotate
}
