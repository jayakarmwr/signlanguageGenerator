...
// N.js
export function animateN(hand, scene) {
    const fingers = {
        thumb: [],
        index: [],
        middle: [],
    ...
    // Apply the 'N' gesture (two fingers draped over the palm)
    fingers.index.forEach(bone => animateBone(bone, 2.5));   // Curl index finger
    fingers.middle.forEach(bone => animateBone(bone, 2.5));  // Curl middle finger
    fingers.ring.forEach(bone => animateBone(bone, 0));      // Extend ring finger
    fingers.pinky.forEach(bone => animateBone(bone, 0));       // Extend pinky finger
