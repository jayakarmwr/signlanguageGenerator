// P.js
export function animateP(hand, scene) {
    const fingers = {
        thumb: [],
        index: [],
        middle: [],
    ...
    // Apply the 'P' gesture
    fingers.index.forEach(bone => animateBone(bone, 0));  // All fingers extended 
    fingers.middle.forEach(bone => animateBone(bone, 0));  // and slightly curled
    fingers.ring.forEach(bone => animateBone(bone, 0));
    fingers.pinky.forEach(bone => animateBone(bone, 0));
    fingers.thumb.forEach(bone => animateBone(bone,0)); // Curl middle finger
}
