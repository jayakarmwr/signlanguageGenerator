...
export function animateA(hand, scene) {
    ...
    fingers.index.forEach(bone => animateBone(bone, 1.3, 0, 0));
    fingers.middle.forEach(bone => animateBone(bone, 1.3, 0, 0));
    fingers.ring.forEach(bone => animateBone(bone, 1.3, 0, 0));
    fingers.pinky.forEach(bone => animateBone(bone, 1.3, 0, 0));

    // Animate all thumb bones
    fingers.thumb.forEach(bone => animateBone(bone, 0, 0, -0.1));
}
