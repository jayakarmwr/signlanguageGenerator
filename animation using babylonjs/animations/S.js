// S.js
export function animateS(hand, scene) {
    const fingers = {
        thumb: [],
        index: [],
        middle: [],
    ...
    // Apply the 'S' gesture (fist with thumb in front)
    fingers.index.forEach(bone => animateBone(bone, 2.5));   // Adduct index finger
    fingers.middle.forEach(bone => animateBone(bone, 2.5));  // Adduct middle finger
    fingers.ring.forEach(bone => animateBone(bone, 2.5));    // Adduct ring finger
    fingers.pinky.forEach(bone => animateBone(bone, 2.5));   // Adduct pinky finger
    fingers.thumb.forEach(bone => animateBone(bone, 0.5));    // Position thumb
}
