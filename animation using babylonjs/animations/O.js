// O.js
export function animateO(hand, scene) {
    const fingers = {
        thumb: [],
        index: [],
        middle: [],
    ...
    // Apply the 'O' gesture (fingers form an O shape)
    fingers.index.forEach(bone => animateBone(bone, 0.5));   // Slightly curve index finger
    fingers.middle.forEach(bone => animateBone(bone, 0.5));  // Slightly curve middle finger
    fingers.ring.forEach(bone => animateBone(bone, 0.5));    // Slightly curve ring finger
    fingers.pinky.forEach(bone => animateBone(bone, 0.5));   // Slightly curve pinky finger
    fingers.thumb.forEach(bone => animateBone(bone, -0.7));    // Adduct thumb (form the O shape)
}
